// ---------------------- deleteFuturePricePlanController.js ----------------------
smartApp.controller('DeleteFuturePricePlanController', function($scope,
    $filter,
    SystemService,
    $routeParams,
    DeleteFuturePricePlanService,
    ReasonService,
    AuthenService,
    ValidateMsgService,
    $ngBootbox) {
    ...
    $scope.divID = "deleteFuturePricePlanContent";
    $scope.isMatch = true;
    $scope.referralcode = "";
    $scope.CitizenID = "";
    ...
    ReasonService.list("78", function(result) {
        ...
        $scope.setDefualtReason($scope.reasons, "CREQ");
            ...
            SystemService.callServicePost($scope.data.orderRequest, headers, function(result) {
                ...
                if (result.status) {
                    ...
                    SystemService.showAlert({
                        ...
                        "th-message": "",
                        "technical-message": "DeleteFuturePricePlanController"
                    });
                }
            });
        }
    ...
    $scope.checkAutoApprove = function() {
        ...
        console.log(data);
        DeleteFuturePricePlanService.ValidateAutoApproveCode(data, function(result) {
            console.log(result);
            SystemService.hideLoading();
    ...
    $scope.onCheckValidateApproveCode = function() {
        ...
        if ($scope.isProcessApproveCode) {
            ...
            if ($scope.data.orderRequest['order']['order-items'][0]['order-data']['IR-APPROVE-CODE']) {
                SystemService.showLoading();
                DeleteFuturePricePlanService.ValidateApproveCodeCallback($scope.data.responseData.customer["installed-products"][0]["company-code"],
    ...
    $scope.onLoad = function() {
        ...
        AuthenService.getAuthen(function(result) {
            ...
            SystemService.getOrderId(ch, dl, function(resultOrder) {
                ...
                if ($scope.SubNo != 'null') {
                    DeleteFuturePricePlanService.validateIRIDDCallback($scope.SubNo, function(resultData) {
    ...
    $scope.futurePricePlan = [{
        "offer-code": "65112",
        "offer-type": "FUTSOC",
        "order-detail": "FUTUREORDER",
        "order-remark": "PLTKAP07->ESVSMP40",
        "create-date": "12/10/2016",
        "effective-date": "31/10/2016",
        "status": "ACTIVE"
    },{
        "offer-code": "11200212",
        "offer-type": "NXTPP",
        "order-detail": "NPTKAP07",
        "order-remark": "PLTKAP07->NPTKAP07",
        "create-date": "06/10/2016",
        "effective-date": "01/11/2017",
        "status": "WAITING"
    }];

    $scope.displayFuturePricePlan = angular.copy($scope.futurePricePlan);
